(function () {
    ...
    function init() {
        ...
        // @ts-ignore
        for (const [ id, container ] of sliders.entries()) {
            ...
            const data = {
                id, isMove, isAsymetric, isOpacity, isAuto,
     ...
     * @returns {Void}
     */
    function setAutoRun(data) {
        if (data.isAuto) {
            clearAutoRun(data);
            timeouts[data.id] = window.setTimeout(() => showItem(data), 10000);
        }
    }

    /**
     * Clears the timeout to move the slider in x seconds
     * @param {Object} data
     * @returns {Void}
     */
    function clearAutoRun(data) {
        if (timeouts[data.id]) {
            window.clearTimeout(timeouts[data.id]);
            timeouts[data.id] = null;
        }
    }
    ...
    window.addEventListener("resize", () => {
        ...
        resizer = window.setTimeout(() => init(), 500);
    });
